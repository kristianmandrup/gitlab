...
/**
 * Get a single repository commit in a project.
 *
 * @param {Object} params
 *  - {String} id - The ID of a project
 *  - {String} sha - The ID of the commit
 * @param {Function} callback
 */
Repository.prototype.getCommit = function (params, callback) {
  params.type = `commits/${params.sha}`;
  this.list(params, callback);
};
...
/**
 * Retrieves the diff of a commit
 *
 * @param {Object} params
 *  - {String} sha (required) - The ID of the commit
 * @param {Function} callback
 */
Repository.prototype.getDiff = function (params, callback) {
  params.type = `commits/${params.sha}/diff`;
  this.client.request('GET', this.path, params, callback);
};

/**
 * Makes a commit with multiple actions
 *
 * @param {Object} params
 *  - {String} branch (required) - The branch to affect (default: master)
 *  - {String} commit_message (required) - Commit message
 *  - {Array} actions (required) - Actions objects (action, file_path, content)
 *  - {String} author_email - e-mail addr of author
 *  - {String} author_name - name of author
 * @param {Function} callback
 *
 * Sample action: (create, update, delete, modify)
 * {
 *   "action": "create",
 *   "file_path": "foo/bar",
 ...
 * }
 *
 * For move: previous_path { String }
 * Encoding: encoding { String } - text or base64. text is default
 */

Repository.prototype.commitActions = function (params, callback) {
  console.log('commitActions', params)
  params.branch = params.branch || 'master'
  params.type = 'commits';
  let method = 'post'
  //   params.method = method
  console.log('post', this.path, params)
  this.client.request(method, this.path, params, callback);
};
